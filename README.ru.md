...
- Для одного пользователя можно включить `gradeLimitPolicy`. Оценка ниже порога меняет только новые запросы и ручные продления; текущая сессия автоматически не меняется.
- При ограничении по оценкам эффективный лимит и жесткий максимум равны `min(дневной лимит, restrictedLimitMinutes)`, поэтому правило никогда не увеличивает доступ, а глобальный grace не добавляется.
- Устаревший отчет с плохой оценкой сохраняет ограничение. Устаревший нормальный отчет или некорректный источник блокирует новые запросы, но не останавливает активную сессию.
...
Текущая версия разрешает включить `gradeLimitPolicy` не более чем для одного пользователя, поскольку `currentGrades.json` содержит один снимок. `normalDecisionTtlMinutes: 20160` соответствует 14 дням. UI получает настроенные порог и TTL из `/api/state`; эти значения не зашиты в код отображения.

Снимок оценок должен содержать `userName`, разбираемый `asOf` и массив `currentGrades`. Для `asOf` поддерживаются ISO 8601 и `MM/dd/yyyy hh:mm tt` в часовой зоне приложения. `averagePercentage` задается дробью от `0` до `1`; значение `null` правило игнорирует.
...
- `GET /api/current-grades` — текущий снимок оценок с `userName` для главной страницы.
